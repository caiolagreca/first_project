...
export class RepositoryBase<T> {
  ...
  public checkAdmin(config?: any): void {
    if (!config?.isAdmin) throw new Error("REPO_AUTH_DENIED");
  }
  ...
  public async find<D>(
    filter: any,
    project?: ProjectionType<T>,
    options?: { sort?: any; skip?: any; limit?: any },
  ): Promise<D[]> {
    project = project || { __v: 0 };
  ...
  public async findOneAndUpdate<D>(
    filter: any,
    data: UpdateQuery<T>,
